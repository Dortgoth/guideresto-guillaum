...
public class GradeService {
    ...
    public boolean createGrade(Grade grade) {
        ...
        try {
            connection.setAutoCommit(false); // Start a transaction
            boolean success = gradeMapper.insert(grade);
            if (success) {
    ...
    public boolean updateGrade(Grade grade) {
        ...
        try {
            connection.setAutoCommit(false); // Start a transaction
            boolean success = gradeMapper.update(grade);
            if (success) {
    ...
    public boolean deleteGrade(int gradeId) {
        ...
        try {
            connection.setAutoCommit(false); // Start a transaction
            boolean success = gradeMapper.delete(gradeId);
            if (success) {
    ...
    public Grade findGradeById(int gradeId) {
        ...
        try {
            connection.setAutoCommit(false); // Start a transaction
            Grade grade = gradeMapper.findById(gradeId);
            connection.commit(); // Commit the transaction
            return grade;
        } catch (SQLException e) {
            try {
